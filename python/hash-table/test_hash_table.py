...
def test_should_report_length_of_empty_table():
...
def test_should_not_create_none_values():
...
def test_should_raise_error_when_deleting(hash_table):
    with pytest.raises(KeyError) as exception_info:
...
def test_should_update_value(hash_table):
    ...
    hash_table["hola"] = "goodbye"
    assert hash_table["hola"] == "goodbye"
    assert len(hash_table) == 3
...
def test_should_return_duplicate_values():
    ...
    hash_table["Alice"] = 24
    hash_table["Bob"] = 25
    hash_table["Charlie"] = 25
    assert [24, 25, 25] == unordered(hash_table.values)
